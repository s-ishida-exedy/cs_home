...
<asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:EXPDBConnectionString %>" SelectCommand="SELECT DISTINCT [T_EXL_CSANKEN].[INVOICE] AS C FROM [T_EXL_CSANKEN] LEFT JOIN [T_EXL_CSKANRYO] ON [T_EXL_CSANKEN].[INVOICE] = [T_EXL_CSKANRYO].[INVOICE] WHERE [T_EXL_CSANKEN].[INVOICE] IS NOT NULL and [T_EXL_CSANKEN].[INVOICE] <>'' AND [T_EXL_CSANKEN].LCL_QTY <>'LCL' AND [T_EXL_CSANKEN].[FIN_FLG] <>'1' AND [T_EXL_CSANKEN].[FORWARDER] <>'上野  ' ORDER BY C "></asp:SqlDataSource>
